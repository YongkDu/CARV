"""
GPT evaluator for the human annotation study.

Reads model responses from annotation/annotation_data/ — the same response
subset that human annotators labeled — and runs GPT-4o with the 'overall'
prompt to produce failure_stage labels. Comparing these labels against human
annotations yields the inter-annotator agreement (Cohen's kappa) that validates
the automatic GPT evaluator in the paper.

Usage:
    Set model_response_path to annotation/annotation_data/ in __main__, then:
        python analysis_3step_annotation.py
"""
import json
import os
import base64
...
LABEL_ROOT = f"/data/yongka/analogy/spatial/data_labels4"
...
def get_response_from_gpt(api_config, prompt):
    ...
    elif api_config["model"] == "o1" or  api_config["model"] == "gpt-5-mini":
        try:
            client = OpenAI(api_key=api_config["api_key"])
            response = client.chat.completions.create(
...
def analyze_with_gpt(response_file, label_file, analyze_file, api_config, operation, img_root):
    ...
    for i,item in enumerate(responses):
        if 'subject' in response_file and analysis_result[i].get('failure_stage', None) is not None:
...
def analyze_compositional_files(model_response_path, api_config, tasks, models, difficulties, operation, data_size):
    analyze_folder = "/data/yongka/analogy/spatial/annotation/analysis"
    ...
    for i in range(1):
        for task in tasks:
            ...
            for oper in operation:            
                response_file = model_response_path + f"/{task}_{oper}.jsonl"
                analyze_file = f"{analyze_folder}/{task}_{oper}_{7}.jsonl"
...
if __name__ == "__main__":
    ...
    model_response_path = f"/data/yongka/analogy/spatial/annotation/annotation_data"
    # analysis_path = f"/data/yongka/analogy/spatial/analysis_{ot}"
    analyze_compositional_files(model_response_path, api_config, task_category, None, None, operations, data_size)